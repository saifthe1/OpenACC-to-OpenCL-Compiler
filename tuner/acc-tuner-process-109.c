...
float eval_matmul_performance(void * params, long start, long end) {
  size_t n = *(size_t *)params;
  size_t m = *(size_t *)(params + sizeof(size_t));
  size_t p = *(size_t *)(params + 2 * sizeof(size_t));

  return 2. * ((n * m) / (1. * (end - start))) * p;
}
...
void acc_tuner_generate_performance_per_param_table() {
  ...
  for (m_exp = 5; m_exp < 14; m_exp++)
  for (p_exp = 5; p_exp < 14; p_exp++) {
    perf_data->min_run_id = -1; 
    ...
    sprintf(query, "SELECT run_id,version_id,gflops FROM Performances WHERE n=='%zd' AND m=='%zd' AND p=='%zd'", power_of_2[n_exp], power_of_2[m_exp], power_of_2[p_exp]);

    status = sqlite3_exec (acc_tuner->versions_db, query, &acc_tuner_param_perf_callback, perf_data, &err_msg);
    assert(status == SQLITE_OK);

    if (perf_data->cnt > 0) {
      sprintf(query, "INSERT INTO ParamPerf Values ( '%zd', '%zd', '%zd', '%zd', '%zd', '%f', '%zd', '%zd', '%f', '%f' );",
...
int main(int argc, char ** argv) {
  ...
  acc_tuner_generate_performance_table(&eval_matmul_performance);
