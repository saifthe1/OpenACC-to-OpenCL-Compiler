...
size_t loop_length_vectadd(size_t loop_id, void * params_) {
  assert(loop_id == 0);
  assert(params_ != NULL);
...
void acc_tuner_load_gen_data(
  ...
  size_t max_num_params_values = params_exp_max - params_exp_min + 1;
  gen_data->params_values = malloc(max_num_params_values * sizeof(size_t));
  gen_data->num_params_values = 0;
  for (exp = params_exp_min; exp <= params_exp_max; exp++)
    if (power_of_2[exp] * sizeof(float) <= max_alloc_size) {
      ((size_t *)gen_data->params_values)[gen_data->num_params_values] = power_of_2[exp];
      gen_data->num_params_values++;
    }
  gen_data->params_values = realloc(gen_data->params_values, gen_data->num_params_values * sizeof(size_t));

  gen_data->num_portions = 1;

  gen_data->loop_length_func = &loop_length_vectadd;
}
